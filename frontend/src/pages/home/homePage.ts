...
export class HomePage extends HTMLElement {
    ...
    /**
     * Obtiene los productos, categorías y marcas desde la API.
     *
    ...
    loadData = async (limit: number = 10, page: number = 1) => {
        ...
        const optionsMarks: string = marks.ok
            ? marks.data.data.map((mark) => `{"label":"${mark.name}","type":"checkbox","checked":false}`).join(',')
            : `{"label": "No fue posible cargar las marcas", "type": "checkbox", "checked": false}`;

        const productsData: ProductResponseApi | string = products.ok ? products.data : products.data;

        return { optionsCategories, optionsMarks, productsData };
    }
